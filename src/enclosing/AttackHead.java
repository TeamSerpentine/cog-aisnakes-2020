...
/**
 * a snake that goes after the head of the opponent (or rather, a tile near the head)
 */
public class AttackHead implements Bot {
    ...
    private Map <Coordinate, Coordinate> opponent_cameFrom_old = new HashMap<Coordinate, Coordinate>();
    private HashMap <Coordinate, Integer> opponent_depth_old = new HashMap<Coordinate, Integer>();

    private Coordinate snake_head;
    private Coordinate opponent_head;
    ...
    public Direction chooseDirection(Snake snake, Snake opponent, Coordinate mazeSize, Coordinate apple) {
        ...
        opponent_head = opponent.getHead();
        ...
        // for enclosing:
        makeOldBFSGraphForOpponent(snake, opponent, mazeSize, apple);
    ...
    private Direction chooseMove(Snake snake, Snake opponent, Coordinate mazeSize, Coordinate apple) {
        ...
        Coordinate opponentTowardsCenter = findTileNextToOpponentTowardsCenter(snake, opponent, mazeSize);
        if (opponentTowardsCenter.inBounds(mazeSize)) {
            move = pathTo(opponentTowardsCenter);
        }

        if (move == null) {
            System.out.println("attackbot can't go to opponent head location");
        ...
        if (encloseMove != null) {
            move = encloseMove;
            System.out.println("attackbot is trying to enclose!");
        }
    ...
    /**
     * finds the closest tile which is not occupied by our snake or the opponent snake
     * @param nonEmptyTile the tile you want to search in the neighborhood of
     * @param snake our snake
     * @param opponent opponent snake
     * @param mazeSize board
     * @return an empty tile if there is one, otherwise null
     */
    private Coordinate findClosestEmptyTile(Coordinate nonEmptyTile, Snake snake, Snake opponent, Coordinate mazeSize) {
        if (isEmptyTile(nonEmptyTile, snake, opponent)) {
            // the tile actually was empty
            return nonEmptyTile;
        } else {
            // first check towards center (useful for enclosing, probably)
            Coordinate center = findCenter(mazeSize);
            Direction centerDirection = betterDirectionFromTo(nonEmptyTile, center);
            if (isEmptyTile(nonEmptyTile.moveTo(centerDirection), snake, opponent)) {
                return nonEmptyTile.moveTo(centerDirection);
            }
            // then check other neighbouring tiles
            for (Direction dir : DIRECTIONS) {
                if (dir != centerDirection) { // already checked that one
                    if (isEmptyTile(nonEmptyTile.moveTo(dir), snake, opponent)) {
                        return nonEmptyTile.moveTo(dir);
                    }
                }
            }
            // if those were all occupied, just check the whole board
            int distance;
            int minimum_distance = mazeSize.x + mazeSize.y + 100; // big enough
            Coordinate closest_empty_tile = null;
            for (int x=0; x < mazeSize.x; x++) {
                for (int y=0; y < mazeSize.y; y++) {
                    if (isEmptyTile(new Coordinate(x,y), snake, opponent)) {
                        distance = Math.abs(nonEmptyTile.x - x) + Math.abs(nonEmptyTile.y - y);
                        if (distance < minimum_distance) {
                            minimum_distance = distance;
                            closest_empty_tile = new Coordinate(x,y);
                        }
                    }
                }
            }
     ...
     * (meaning not part of a snake body)
     * @param tile coordinate to check
     * @param snake our snake
     * @param opponent other snake
     * @return true if empty, false if not
     */
    private boolean isEmptyTile(Coordinate tile, Snake snake, Snake opponent) {
        return !snake.elements.contains(tile) && !opponent.elements.contains(tile);
    }


    /**
     * finds the center tile, or one really close if there isn't really one center tile
     * (for example, with a board of 14 wide (so index 0,1,2,...,13) the middle is in between 6 and 7.
     * in that case we choose 7 here. No particular reason.)
     * @param mazeSize the board
     * @return coordinate of center
     */
    private Coordinate findCenter(Coordinate mazeSize) {
        int centerX;
        int centerY;
        if (mazeSize.x % 2 == 0) {
            centerX = mazeSize.x / 2;
        } else {
            centerX = (mazeSize.x - 1) / 2;
        }
        if (mazeSize.y % 2 == 0) {
            centerY = mazeSize.y / 2;
        } else {
            centerY = (mazeSize.y - 1) / 2;
        }
        return new Coordinate(centerX, centerY);
    }

    /**
     * finds the tile which is next to the opponents head in the direction of the center of the board
     * @return coordinate of tile
     */
    private Coordinate findTileNextToOpponentTowardsCenter(Snake snake, Snake opponent, Coordinate mazeSize) {
        Coordinate center = findCenter(mazeSize);
        Direction centerDirection = betterDirectionFromTo(opponent_head, center);
        if (isEmptyTile(opponent_head.moveTo(centerDirection), snake, opponent)) {
            return opponent_head.moveTo(centerDirection);
        } else {
            return findClosestEmptyTile(opponent_head.moveTo(centerDirection), snake, opponent, mazeSize);
        }
    }


    /**
     * finds the tile in front of the opponents head (direction from second snake element through its head forward)
     * @param opponent enemy snake
     * @return coordinate, could be out of bounds
     */
    private Coordinate findTileInFrontOfOpponent(Snake opponent) {
        Coordinate afterOpponentHeadNotFinal = null;
        if (opponent.body.size() >= 2) {
            Iterator<Coordinate> it = opponent.body.iterator();
            it.next();
            afterOpponentHeadNotFinal = it.next();
        }
        Coordinate afterOpponentHead = afterOpponentHeadNotFinal;
        assert afterOpponentHead != null;
        Direction opponentForward = directionFromTo(afterOpponentHead, opponent_head);
        return opponent_head.moveTo(opponentForward);
    }
    ...
    private void simpleEnclose(Snake snake, Coordinate mazeSize) {
        ...
        /* The only illegal move is going backwards. Here we are not including it, to get validMoves array */
        ...
        for (Direction validMove : validMoves) {
            nextToHead = snake_head.moveTo(validMove);
            if (nextToHead.inBounds(mazeSize)) {
                if (opponent_depth_old.get(nextToHead) == null) {
                    break;
                } else {
                    int path_depth_here = opponent_depth_old.get(nextToHead);
                    if (path_depth_here > 1) {
                        // depth has to be greater than 1, otherwise we might bump into the opponent's head
                        if (findCoordinatesWithThisDepth(path_depth_here, opponent_depth_old, mazeSize).length == 1) {
                            // there is only 1 path with this depth in it!
                            System.out.println("go step " + validMove + " to enclose!");
                            System.out.println();
                            // watch out: I think directions UP and DOWN are set wrong (LEFT and RIGHT are correct)

                            encloseMove = validMove;
    ...
    /**
     * Makes a simple BFS graph
     * (not taking into account that space will become available once the ass of a snake moves etc.)
     * @param snake    Our snake
     * @param opponent Opponent snake
     * @param mazeSize Size of the board
     * @param apple    Coordinate of an apple
     */
    private void makeOldBFSGraphForOpponent(Snake snake, Snake opponent, Coordinate mazeSize, Coordinate apple) {
        opponent_cameFrom_old.clear();
        opponent_depth_old.clear();
        ...
        opponent_cameFrom_old.put(opponent_head, null); //
        opponent_depth_old.put(opponent_head, 0);
        ...
        while (!queue.isEmpty()) {
            Coordinate start = (Coordinate) queue.get(0);
            Integer depth_level = opponent_depth_old.get(start) + 1; // The depth level
            ...
            for (Direction move : Direction.values()) {
                ...
                if (newPos.inBounds(mazeSize)) { // Skip this if for positions out of the maze
                    if (!arrayVisited[newPos.x][newPos.y]) {
                        queue.add(newPos); // If this place is not visited add it to the queue
                        arrayVisited[newPos.x][newPos.y] = true; // We don't want to visit this place again.
                        if (!opponent_cameFrom_old.containsKey(newPos)) {
                            opponent_cameFrom_old.put(newPos, start); // Puts the location start in newPos so we can trace back how we got to the apple.
                            opponent_depth_old.put(newPos, depth_level);
                        }
                    }
                }
            }
    ...
    /**
     * set a direction from pointA to pointB, which is better defined than the original directionFromTo,
     * because this one goes into the biggest difference coordinate first.
     * (so if difference in x is larger than difference in y, move in x direction first)
     * @param pointA coordinates of starting point
     * @param pointB coordinates of goal
     * @return direction
     */
    public Direction betterDirectionFromTo(Coordinate pointA, Coordinate pointB) {
        Coordinate vector = new Coordinate(pointB.x - pointA.x, pointB.y - pointA.y);
