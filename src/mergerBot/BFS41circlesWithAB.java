...
/**
 * merged Bot on 25 april 2020
 *
 */
public class BFS41circlesWithAB implements Bot {
    private static final Direction[] DIRECTIONS = new Direction[]{Direction.UP, Direction.DOWN, Direction.LEFT, Direction.RIGHT};
    // for alpha-beta
    Bot alphaBetaBot = new alphabeta();
    ...
    private Direction chooseMove(Snake snake, Snake opponent, Coordinate mazeSize, Coordinate apple) {
        ...
        if (circle_type>0) {  // here we are circling
            ...
            dontTakeApple = false;
            // TODO check time limit better
            // geef tijd die over is (starttijd) door aan alphabeta

            if (distanceBetween(snake.getHead(), apple) < maxAppleDistance) {
                move = alphaBetaBot.chooseDirection(snake, opponent, mazeSize, apple);
                System.out.println("alpha beta move");
            } else {
                System.out.println("BFS move, because apple too far");
                // we do BFS here because apple is too far
                makeOpponentBFSGraph(snake, opponent, mazeSize, apple); // We do this before making our BFS because we want to calculate the distance to the apple
                makeBFSGraph(snake, opponent, mazeSize, apple);
                move = pathTo(apple);
