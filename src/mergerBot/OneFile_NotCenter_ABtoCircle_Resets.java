...
/**
 * one file instead of importing alphabeta
 */
public class OneFile_NotCenter_ABtoCircle_Resets implements Bot, Runnable {
    ...
    private int fct_earlyReachableTiles = 4;
    ...
    private Direction chooseMove(Snake snake, Snake opponent, Coordinate mazeSize, Coordinate apple) {
        ...
        if (circle_type > 0 && snake.body.size()>5) {  // here we are circling
            ...
            /****** start of go-towards-circle-with-alphabeta  ***/
            ...
            for (int i = 0; i<circle_positions_length; i++) {
                if (!(distanceBetween(head, apple)-1 <= distanceBetween(circle_positions[i], apple)) && !circle_complete){
                    useAlphaBeta = true;
                }
            }
            ...
            /****** end of go-towards-circle-with-alphabeta  ***/
...
//                }
                percentageCloserToUs = simplePercentageOfTilesCloserToUs(our_snake, opp_snake, mazeSize, apple);
//                distanceToCentre += distanceBetween(snake_head, centre);
            } else { // We ate the apple
                score += scr_WeAteApple + depth_weAteApple;
            }
        }
        return (int) (score + (fct_distanceToApple * distanceToApple + fct_earlyReachableTiles * percentageCloserToUs));
    }
